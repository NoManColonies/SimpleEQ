...
ChainSettings getChainSettings(juce::AudioProcessorValueTreeState &apvts) {
  ...
  settings.lowCutFreg = apvts.getRawParameterValue("LowCut Freq")->load();
  settings.highCutFreg = apvts.getRawParameterValue("HighCut Freq")->load();
  settings.peakFreg = apvts.getRawParameterValue("Peak Freq")->load();
  ...
  settings.peakBypassed =
      apvts.getRawParameterValue("Peak Bypassed")->load() > 0.5f;
...
void SimpleEQAudioProcessor::updatePeakFilter(
  ...
  auto peakCoeffients = makePeakFilter(chainSettings, getSampleRate());
...
void SimpleEQAudioProcessor::updateLowCutFilter(
    const ChainSettings &chainSettings) {
...
void SimpleEQAudioProcessor::updateHighCutFilter(
    const ChainSettings &chainSettings) {
  ...
  rightChain.setBypassed<ChainPositions::HighCut>(
      chainSettings.highCutBypassed);
...
SimpleEQAudioProcessor::createParameterLayout() {
  ...
  layout.add(std::make_unique<juce::AudioParameterChoice>(
      "LowCut Slope", "LowCut Choice", stringArray, 0));
  layout.add(std::make_unique<juce::AudioParameterChoice>(
      "HighCut Slope", "HighCut Choice", stringArray, 0));
